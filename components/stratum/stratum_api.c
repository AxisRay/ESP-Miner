...
{
    if (socket.is_tls) {
        esp_tls_conn_write(socket.tls_socket, msg, strlen(msg));
    } else {
        write(socket.tcp_socket, msg, strlen(msg));
    }
}
...
{
    if (socket.is_tls) {
        return esp_tls_conn_read(socket.tls_socket, buffer, buffer_size);
    } else {
